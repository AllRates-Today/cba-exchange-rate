# Central Bank of Armenia Exchange Rates API — cba-exchange-rate

[![npm version](https://img.shields.io/npm/v/cba-exchange-rate.svg)](https://www.npmjs.com/package/cba-exchange-rate)
...
- 🏛️ **Official published rates** — Central Bank of Armenia's own table, with the publisher's own `rate_date` on every response
...
> **Official rate, not mid-market:** every value here is a number Central Bank of Armenia itself published, fixed once printed and carrying the central bank's own `rate_date` — what filings and audits require. Need the live interbank midpoint for pricing or display instead? Use the [mid-market API](https://allratestoday.com/docs/) or [`@allratestoday/sdk`](https://www.npmjs.com/package/@allratestoday/sdk). The two can diverge by several percent.

## ⚡ Try it without a key

The latest Central Bank of Armenia table is also served keyless, CORS-open and edge-cached, for evaluation, embeds and AI agents:

```bash
curl "https://allratestoday.com/api/open/central-bank/cba?source=USD&target=AMD"
```

```js
const r = await fetch('https://allratestoday.com/api/open/central-bank/cba').then((x) => x.json());
...
```bash
npm install cba-exchange-rate
```

```bash
yarn add cba-exchange-rate
```

```bash
pnpm add cba-exchange-rate
```

Requires Node 18+ (global `fetch`); also runs on Bun, Deno and edge runtimes. Also published under the org scope as [`@allratestoday/cba-exchange-rate`](https://www.npmjs.com/package/@allratestoday/cba-exchange-rate) — same code, same versions.

## 🏁 Quick start

```js
import { getRate } from 'cba-exchange-rate';

const pair = await getRate('USD', 'AMD', { apiKey: 'art_live_...' });
console.log(pair.rate, pair.rate_date); // the official Central Bank of Armenia rate, on the central bank's own date
```
...
```js
import { getLatestRates } from 'cba-exchange-rate';
...
```js
import { getRatesForDate } from 'cba-exchange-rate';

const day = await getRatesForDate('2026-06-30', { apiKey: 'art_live_...' });
// Optionally narrow to one pair:
const one = await getRatesForDate('2026-06-30', { apiKey: 'art_live_...', source: 'USD', target: 'AMD' });
```
...
Central Bank of Armenia currently publishes rates covering **30 currencies** against the AMD (as of the latest table):

🇦🇪 `AED` · 🇦🇺 `AUD` · 🇧🇷 `BRL` · 🇧🇾 `BYN` · 🇨🇦 `CAD` · 🇨🇭 `CHF` · 🇨🇳 `CNY` · 🇨🇿 `CZK` · 🇪🇺 `EUR` · 🇬🇧 `GBP` · 🇬🇪 `GEL` · 🇭🇰 `HKD` · 🇮🇳 `INR` · 🇮🇷 `IRR` · 🇯🇵 `JPY` · 🇰🇬 `KGS` · 🇰🇿 `KZT` · 🇳🇴 `NOK` · 🇳🇿 `NZD` · 🇵🇱 `PLN` · 🇷🇺 `RUB` · 🇸🇪 `SEK` · 🇸🇬 `SGD` · 🇹🇯 `TJS` · 🇺🇦 `UAH` · 🇺🇸 `USD` · 🇺🇿 `UZS` · `XAG` · `XAU` · `XDR`
...
- Publisher's own page: [Exchange rate archive](https://old.cba.am/en/SitePages/ExchangeArchive.aspx) · [www.cba.am](https://www.cba.am)
- Publication: every business day; the exact schedule, freshness status and any current delay are on the [Central Bank of Armenia rates page](https://allratestoday.com/central-bank-rates-api/cba/)
...
- Central Bank of Armenia quotes **AMD per 1 unit of foreign currency** (e.g. `base: "USD", quote: "AMD"` means AMD per one US dollar).
...
Loading the official Central Bank of Armenia rate into an accounting system is a supported workflow, not a hack. Step-by-step guides with the direction each system expects:
...
```bash
curl "https://allratestoday.com/api/v1/central-bank/cba/latest?format=xml&api_key=art_live_..."
```
...
If Central Bank of Armenia does not print a pair directly, the API resolves it from the central bank's own table and says so — official and computed values are never confused:

| `method` | `derived` | Meaning |
| --- | --- | --- |
| `published` | `false` | The central bank printed this pair directly |
| `inverse` | `true` | Computed as 1 ÷ the published opposite direction |
| `cross` | `true` | Computed via AMD from two published rates |
...
| `404` | Pair or date range not covered by Central Bank of Armenia |
...
```ts
import type { LatestRates, PairRate, DatedRates, RateEntry, HistoryQuery, RequestOptions } from 'cba-exchange-rate';
```

## 📦 CommonJS

```javascript
const { getRate } = require('cba-exchange-rate');

getRate('USD', 'AMD', { apiKey: 'art_live_...' }).then((pair) => console.log(pair.rate));
```
...
- Hugging Face: [AllRates/central-bank-exchange-rates](https://huggingface.co/datasets/AllRates/central-bank-exchange-rates) — one CSV per institution (`rates/cba.csv`)
- Kaggle: [allratestoday/central-bank-exchange-rates](https://www.kaggle.com/datasets/allratestoday/central-bank-exchange-rates)
- CDN JSON: `https://cdn.jsdelivr.net/gh/AllRates-Today/central-bank-exchange-rates@main/data/cba/latest.json`

## 🔗 Links

- [Central Bank of Armenia rates page](https://allratestoday.com/central-bank-rates-api/cba/) — live table, publication cadence, FAQ
- [All central bank sources](https://allratestoday.com/central-bank-rates-api/)
- [Package docs on the site](https://allratestoday.com/docs/sdk/cba-exchange-rate/) · [ERP integration guides](https://allratestoday.com/docs/integrations/)
...
- [GitHub](https://github.com/AllRates-Today/cba-exchange-rate)
